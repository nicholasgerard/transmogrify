...
| --- | --- | --- |
| The app cannot resume a thread and reports `invalid transport in mcp_servers.codex_app` | App versions `26.901.51231` (build `8109`) and `26.903.61454` (build `8378`) reject their own disabled remote app-tools placeholder while attached to a shared runtime, even though lanes stream live. The user's config file need not contain that entry. | If persistence is on, run `node "$SKILL_ROOT/scripts/desktop-attach.js" unpersist --authorize`; then quit and reopen the app normally so it uses its own runtime. |
| Verifying a new app build | A live connection and live streaming do not prove thread resume. | Launch the app attached by hand, watch a lane stream, then open an existing thread in the app and confirm its log shows no resume error; only then run `--verified-build <version> <build>`. |
